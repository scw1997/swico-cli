...
const initHusky = async (targetPath) => {
    ...
    const handleErr = (reject) => {
        const errMsg = chalk.red('Errors occurred while Husky initialization!');
        spinner.fail(errMsg);
        process.exit(1);
...
const initGit = async (targetPath) => {
    //报错处理
    const handleErr = (reject) => {
        const errMsg = chalk.red('Errors occurred while Git initialization!');
        spinner.fail(errMsg);
        process.exit(1);
        return reject(errMsg);
    };
...
const handlePrompt: () => Promise<{
    templateType: string;
    npmType: string;
    needGitHooks: boolean;
}> = async () => {
    const { templateType, npmType, needGitHooks } = await inquirer.prompt([
        // 选择模板类型
        {
            ...
            choices: [
                {
                    name: 'React 19 + Typescript 5.8',
                    value: 'react'
                },
                {
                    name: 'Vue 3.5 + Typescript 5.8',
                    value: 'vue'
                }
            ]
        },
